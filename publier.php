...
<?php //Insertion dans la base de données
  if(isset($_POST['auteur_billet']) && !empty($_POST['auteur_billet']) && isset($_POST['titre_billet']) && !empty($_POST['titre_billet']) && isset($_POST['contenu_billet']) && !empty($_POST['contenu_billet'])) {
    if(isset($_POST['auteur_billet']) && !empty($_POST['auteur_billet'])){  $auteur = htmlspecialchars($_POST['auteur_billet']); } else { echo "<p style='color:red'>Écrivez votre nom.</p>"; }
    if(isset($_POST['titre_billet']) && !empty($_POST['titre_billet'])){  $titre = htmlspecialchars($_POST['titre_billet']); } else { echo "<p style='color:red'>Écrivez un titre.</p>"; }
    if(isset($_POST['contenu_billet']) && !empty($_POST['contenu_billet'])){  $contenu = $_POST['contenu_billet']; } else { echo "<p style='color:red'>Écrivez votre texte.</p>"; }
